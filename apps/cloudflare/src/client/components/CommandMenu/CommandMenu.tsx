...
export default function CommandMenu() {
  ...
  const filesForUser = useQuery(["recentFiles"], getRecentFiles, {
  ...
  const filesForCurrentTab = useMemo(() => {
    switch (currentTab) {
      case "recent":
  ...
  }, [currentTab, filesForUser.data]);

  const fuse = useMemo(() => {
    return new Fuse(filesForCurrentTab, {
      keys: ["title"],
    });
  }, [filesForCurrentTab]);

  const searchResult = useMemo(() => {
    if (search === "") return filesForCurrentTab;
    return fuse.search(search).map((res) => res.item);
  }, [filesForCurrentTab, fuse, search]);
      ...
      <Dialog.Portal>
        <Dialog.Overlay className={s.overlay}>
          ...
          >
            ...
            >
              <Dialog.Title className={s.visuallyHidden}>
                Command menu
              </Dialog.Title>
              <input
                value={search}
