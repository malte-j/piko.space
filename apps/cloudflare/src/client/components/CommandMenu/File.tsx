...
import { removeRecentFile, type RecentFile } from "../../utils/api";
...
export function DeleteButton({ fileId }: { fileId: string }) {
  ...
  const deleteFileRpc = useMutation(removeRecentFile, {
    onSuccess() {
      queryClient.setQueryData<RecentFile[]>(["recentFiles"], (files) => {
  ...
  function deleteFile(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    const deletion = indexedDB.deleteDatabase("ydoc_" + fileId);
    deletion.onerror = () => console.warn("Unable to delete the local document copy");
